...
@role_required('admin')
def admin():
    utilisateurs = list(db_utilisateurs.find({}, {"nom": 1, "role": 1}))
    return render_template('admin/acceuil.html', utilisateurs=utilisateurs)

@app.route('/changer_role/<user_id>', methods=['POST'])
@role_required('admin')
def changer_role(user_id):
    """ Permet à l'admin de modifier le rôle d'un utilisateur. """
    nouveau_role = request.form.get('role')
    
    if nouveau_role not in ['utilisateur', 'admin']:
        flash("Rôle invalide.", "danger")
        return redirect(url_for('admin'))

    db_utilisateurs.update_one({'_id': ObjectId(user_id)}, {'$set': {'role': nouveau_role}})
    flash("Le rôle a été mis à jour avec succès.", "success")
    return redirect(url_for('admin'))
